...
import { Center, Clone, Float, Text, useFBX, useGLTF } from '@react-three/drei'
...
function ToolModel({ tool }: { tool: ToolItem }) {
  ...
  return (
    <Float speed={1} rotationIntensity={0.22} floatIntensity={0.65}>
      <group position={position}>
        <mesh position={[0, 0, -0.35]} scale={[1.45, 0.95, 1]}>
          <sphereGeometry args={[1, 32, 32]} />
          <meshBasicMaterial color={tool.glow} transparent opacity={0.05} />
        </mesh>

        <mesh position={[0, 0, -0.18]} scale={[0.95, 0.62, 1]}>
          <sphereGeometry args={[1, 32, 32]} />
...
function LogoCloud({ tools }: { tools: ToolItem[] }) {
  const group = useRef<Group>(null)
  ...
  useFrame((state) => {
    if (!group.current) return
    group.current.rotation.y += (state.pointer.x * 0.12 - group.current.rotation.y) * 0.025
  ...
  return (
    <group ref={group}>
      {layout.map((item) => (
        <FloatingLogo key={item.tool.name} tool={item.tool} position={item.position} />
      ))}
    </group>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="h-[520px] w-full">
        <Canvas camera={{ position: [0, 0.2, 13.5], fov: 30 }}>
